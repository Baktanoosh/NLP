...
class PerceptronClassifier(SentimentClassifier):
    ...
    def perceptron(self):
        n = len(self.train_ex)
        m = self.extract.get_indexer_len()
        print("n ",n)
        print("m ",m)
        ...
        for i in range(self.epoch):
            acc = np.zeros([n])
            for j in range(n):
                feat = self.extract.extract_features(self.train_ex[j].words)
                pred = float(np.dot(weight_vector,feat) > 0)
                if pred == self.train_ex[j].label:
                    acc[j] = 1
                    continue
                else:
                    if pred == 0 and self.train_ex[j].label == 1:
    ...
    def predict(self, sentence: List[str]):
        feat = self.extract.extract_features(sentence)
