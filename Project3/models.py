...
def train_rnn_classifier(args, train_cons_exs, train_vowel_exs, dev_cons_exs, dev_vowel_exs, vocab_index):
    ...
    for i, train_ex in enumerate(contex):
        ...
        train_ex = [vocab_index.index_of(char) for char in train_ex]
...
class RNNLanguageModel(LanguageModel, nn.Module):
    ...
    def get_next_char_log_probs(self, context):
        ...
        esx = torch.from_numpy(np.array(contex_ind)).long()
        prob = self.forward(esx)
        return self.softmax(prob[-1]).detach().numpy()
...
def train_lm(args, train_text, dev_text, vocab_index):
    ...
    for i in range(len(train_text)):
        chunk_ind = i + chunk_len
        shifted_chunk = 1 + chunk_ind
        trained = train_text[i:chunk_ind]
        label = train_text[i+1:shifted_chunk]
        if len(trained) < chunk_len:
            break
        elif len(label) < chunk_len:
            break
        train_data.append(([vocab_index.index_of(char) for char in trained], [vocab_index.index_of(char) for char in label]))
    ...
    for epoch in range(0, num_epochs):
        ...
        for idx in range(len(train_data)):
            ex, label = train_data[idx] 
            model_rnn_lan.zero_grad()
            ex_tensor = torch.from_numpy(np.array(ex)).long()
            log_probs = model_rnn_lan.forward(ex_tensor)
            y = torch.LongTensor(label).long()
            loss = ce_loss(log_probs, y)
            loss.backward()
            optimizer.step()
        now = time.time()
        print("Total Traing time", now-start)
