...
def train_rnn_classifier(args, train_cons_exs, train_vowel_exs, dev_cons_exs, dev_vowel_exs, vocab_index):
    ...
    optimizer = optim.Adam(model_rnn.parameters(), lr= initial_learning_rate, weight_decay= initial_weight_decay)
...
def train_lm(args, train_text, dev_text, vocab_index):
    ...
    for i in range(len(train_text)):
        train_ex = train_text[i:i+20]
        label = train_text[i+1:i+21]
        if len(train_ex) != chunk_len or len(label) != chunk_len:
            break
        train_data.append((model_rnn_lan.char_embedding(train_ex), model_rnn_lan.char_embedding(train_ex)))

    optimizer = optim.Adam(model_rnn_lan.parameters(), lr= initial_learning_rate, weight_decay= initial_weight_decay)
    for epoch in range(0, num_epochs):
        random.shuffle(train_data)
