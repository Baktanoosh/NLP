...
class RNNClassifier(ConsonantVowelClassifier, nn.Module):
    ...
    def predict(self, context):
        ...
        esx = torch.from_numpy(np.array(contex_ind)).log()
...
def train_rnn_classifier(args, train_cons_exs, train_vowel_exs, dev_cons_exs, dev_vowel_exs, vocab_index):
    ...
    for epoch in range(0, num_epochs):
        ...
        total_loss = 0.0
        for idx in ex_indices[0:2]:
            rnn_model.zero_grad()
            log_probs = rnn_model.forward(train_data[idx][0])
            loss = torch.neg(log_probs).dot(train_data[idx][1])
            total_loss += loss
            loss.backward()
            optimizer.step()
    now = time.time()
