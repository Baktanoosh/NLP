...
class RNNClassifier(ConsonantVowelClassifier, nn.Module):
    ...
    def __init__(self, dict_size, hidden_size, embedding_size, num_layers, cons_size):
    ...
    def forward(self, input):
        ...
        return self.log_softmax(hidden_state)
...
def train_rnn_classifier(args, train_cons_exs, train_vowel_exs, dev_cons_exs, dev_vowel_exs, vocab_index):
    ...
    for i in range(len(contex)):
        if i < cons_size:
            ...
            label = 1
        label = torch.FloatTensor([[label]]) 
        train_data.append((contex, label))
    rnn_model = RNNClassifier(dict_size, hidden_size, embedding_size, num_layers, cons_size)
